...
describe("cli", () => {
  ...
  it("parses lists database path", async () => {
    const calls: string[] = [];
    const program = createProgram(handlers(calls));
  ...
  it("lists from empty default app state does not create the app directory or database", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-home-"));
    const output: string[] = [];

    try {
      await withProcessEnv({ HOME: home }, async () => {
        await createDefaultHandlers(io(output)).lists({});
      });
  ...
  it("lists from an existing app directory without a default database does not create the database", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-home-"));
    const output: string[] = [];
    const appDirectory = path.join(home, ".stargazer");
    const databasePath = path.join(appDirectory, "github-stars.sqlite");
  ...
  it("search after sync reads the same default database from another working directory", async () => {
    ...
    try {
      ...
      expect(output.join("")).toContain("octo/sqlite-tool");
      expect(output.join("")).toContain("[TypeScript]");
      expect(fs.existsSync(path.join(home, ".stargazer", "github-stars.sqlite"))).toBe(true);
  ...
  it("lists after sync reads the same default database from another working directory", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-home-"));
    const firstCwd = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-cwd-a-"));
    const secondCwd = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-cwd-b-"));
    const output: string[] = [];
  ...
  it("lists with an explicit missing database path initializes that database", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-db-"));
    const dbPath = path.join(tempDir, "explicit.sqlite");
    const output: string[] = [];
  ...
  it("lists reads populated data from an explicit database override", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-db-"));
    const dbPath = path.join(tempDir, "explicit.sqlite");
    const output: string[] = [];
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(mockGithubFetch());
  ...
  it("lists reads populated local data without contacting GitHub", async () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "stargazer-db-"));
    const dbPath = path.join(tempDir, "explicit.sqlite");
    const output: string[] = [];
    const fetchSpy = vi.spyOn(globalThis, "fetch").mockImplementation(mockGithubFetch());
...
function mockGithubFetch(
  ...
  const listPayload =
    repositories.length === 0
      ? { data: { viewer: { lists: { nodes: [], pageInfo: { hasNextPage: false, endCursor: null } } } } }
      : listsFixture;
